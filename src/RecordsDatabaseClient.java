...
public class RecordsDatabaseClient extends Application {
    ...
    private TableView<MyTableRecord> result = new TableView<MyTableRecord>(); // Result list of content
	...
	public void initializeSocket(){
        try {
            clientSocket = new Socket(fsd.week1.layoutgrid.Credentials.HOST, fsd.week1.layoutgrid.Credentials.PORT);
    ...
    public void reportServiceOutcome() {
        try {
			...
			// Iterate over the serviceOutcome to update table contents
			String tmp = "";
			serviceOutcome.beforeFirst();
			while (serviceOutcome.next()) {
				// Assuming MyTableRecord is a class representing a row in the table
				MyTableRecord record = new MyTableRecord();
				// Populate record fields from serviceOutcome columns
				// Example:
				...
				record.setCopyID(serviceOutcome.getString(5));
				// Add the record to tmpRecords
				tmpRecords.add(record);
				tmp = tmp + serviceOutcome.getString(1) + " | " + serviceOutcome.getString(2) + " | " + serviceOutcome.getString(3) + " | " + serviceOutcome.getString(4) + " | " + serviceOutcome.getString(5) + "\n";
			}
    ...
    public void execute(){
        ...
        userCommand = artist + ";" + recordshop; // Finished
        ...
        {// Raised if connection is refused or other technical issue
            System.out.println("Client: Exception " + e); // Error is reached
        }
    }
    ...
    public void start(Stage primaryStage) {
		...
		result.getColumns().addAll(titleCol, labelCol, genreCol, rrpCol, copyIDCol);
		GridPane.setConstraints(result, 0, 3, 2, 1);
		grid.getChildren().add(result);
		// Was provided in the original code, however I think it is redundant
